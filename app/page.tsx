...
export default function QuantumAIPortfolio() {
  ...
  const handleProjectClick = (p: Project) => {
    // Explicit copy to satisfy strict inference on some CI builds
    setOpenProject({ ...p });
  };
  ...
  return (
    <div className="min-h-screen font-sans bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-6 py-8">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center text-2xl font-bold">Q</div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold">Quantum + AI Lab</h1>
              <p className="text-sm text-white/70">Crazy 3D front-end · Explainable science · Open innovation</p>
            </div>
          </div>
          ...
          <nav className="flex items-center gap-4">
              ...
              {nav.map((n) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm font-medium ${
                    active === n ? "bg-white/12" : "hover:bg-white/6"
                  }`}
            ...
            <a
              ...
              className="ml-2 px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-400 text-black font-semibold text-sm"
            >
              Get Involved
            </a>
          </nav>
        ...
        <header className="flex flex-col items-center justify-center text-center py-24">
          ...
          >
            Step into a hyper-real lab of quantum computing, AI consciousness, and future-shaping technology.
          </motion.p>
          <div className="mt-8 flex gap-4 flex-wrap justify-center">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-4 rounded-2xl backdrop-blur-xl bg-white/10 border border-white/20 shadow-lg hover:bg-white/20">
              Enter Projects
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-4 rounded-2xl backdrop-blur-xl bg-gradient-to-r from-purple-500 to-cyan-400 text-black font-semibold">
              Explore Labs
            </motion.button>
          </div>
        </header>
        ...
        <main className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section className="lg:col-span-2">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p className="mt-4 text-lg text-white/80">
                Interactive demos, plain-English explainers, and production-ready tooling to let curious people (and researchers) explore quantum computing and AI. No PhD required.
              </p>

              <div className="mt-6 flex gap-3 flex-wrap">
                <button onClick={() => setActive("projects")} className="px-5 py-3 rounded-2xl bg-white/10 hover:bg-white/14">
                  Explore Projects
                </button>
                <button onClick={() => setActive("demos")} className="px-5 py-3 rounded-2xl border border-white/10">
                  Try a Demo
                </button>
                <a href="#glossary" onClick={() => setActive("glossary")} className="px-5 py-3 rounded-2xl bg-white/6">
                  Quick Glossary
                </a>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {projects.map((p) => (
                  <motion.article
                    ...
                    onClick={() => setOpenProject(p as unknown as Project)}
                  >
          ...
          <aside className="space-y-4">
            ...
            <div className="p-4 bg-white/4 rounded-xl">
              ...
              <a href="#contact" onClick={() => setActive("contact")} className="mt-3 inline-block px-3 py-2 rounded-md bg-white/6">
                Contact
              </a>
            </div>
          </aside>
        ...
        {/* Sections that can be activated via nav */}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                {projects.map((p) => (
                  <article key={"full-" + p.id} className="p-4 bg-white/6 rounded-lg">
                    ...
                    <p className="text-sm text-white/70 mt-2">{p.short}</p>
                    <ul className="mt-2 text-xs list-disc pl-4 text-white/60">
                      {p.bullets.map((b) => (
                        <li key={b}>{b}</li>
                      ))}
                    </ul>
                    <div className="mt-3">
                      <button onClick={() => setOpenProject(p as unknown as Project)} className="px-3 py-2 rounded-md bg-white/8">
                        Open
                      </button>
                    </div>
                  </article>
      ...
      <div className="fixed right-4 bottom-4 z-50">
        <button title="Increase text" onClick={() => (document.documentElement.style.fontSize = "18px")} className="px-3 py-2 rounded bg-white/6">
          A+
        </button>
      </div>
    </div>
